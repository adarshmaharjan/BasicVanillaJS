...
window.addEventListener('DOMContentLoaded', function(){
  // console.log('why the fuck r u not loading');
...
function displayMenueBtns(){
  const catagory = menu.reduce(function(values, item){
    if(!values.includes(item.category))
      values.push(item.category);
    return values;
  },['all']);
  // console.log(catagory);
  const catagoryBtns = catagory.map(function(catagory){

    return `
      <button class="filter-btn" type="button" data-id=${catagory}>${catagory}</button>`;
  ...
  filterBtns.forEach(function(btn){
    btn.addEventListener('click', function(e){
      ...
      const menuCatagory = menu.filter(function(menuItems){
        // console.log(menueItems.catagory);
        if(menuItems.category === catagory)
          return menuItems;
      });
      ...
      if(catagory === 'all')
        ...
        displayMenueItems(menuCatagory);
    });
  });
}
